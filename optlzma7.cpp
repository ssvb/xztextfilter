...
struct LZMAEncoder {
    ...
    void EncodeLen(LZMAProbabilities::LenTree& tree, uint32_t len, uint32_t posState) {
        uint32_t symbol = len - MIN_MATCH;
        if (symbol < 8) { EncodeBit(tree.choice1, 0); EncodeTree(&tree.low[posState * 16], 3, symbol); }
        else if (symbol < 16) { EncodeBit(tree.choice1, 1); EncodeBit(tree.choice2, 0); EncodeTree(&tree.mid[posState * 16], 3, symbol - 8); }
        else { EncodeBit(tree.choice1, 1); EncodeBit(tree.choice2, 1); EncodeTree(tree.high.data(), 8, symbol - 16); }
    }
...
void CompressLZMA(const std::vector<uint8_t>& data, std::ostream& out_stream, const LZMAParams& params) {
    ...
    // Scale best_dist arrays to accommodate match lengths calculated in pure bits
        ...
        for (uint32_t i = 0; i < chunk_len; i++) {
            ...
            for (int k = rank; k > 0; k--) {
                ...
                if (sa_pos < (saidx_t)global_pos) {
                    ...
                    // Count matching upper bits of the subsequent byte
                    uint32_t match_bits = 0;
                    if (len_bound < max_limit) {
                        uint8_t a = data[global_pos + len_bound];
                        uint8_t b = data[global_pos - dist + len_bound];
                        while (match_bits < 8 && ((a >> (7 - match_bits)) & 1) == ((b >> (7 - match_bits)) & 1)) {
                            match_bits++;
                        }
                    }
            ...
            if (lit_price < opts[i + 1].price) {
                opts[i + 1].price = lit_price;
                opts[i + 1].pos_prev = i;
                opts[i + 1].back_prev = 0xFFFFFFFF;
            ...
            if (max_limit >= MIN_MATCH) {
                        ...
                        if (len >= MIN_MATCH || (rep_index == 0 && len == 1)) {
                            ...
                            if (rep_index == 0) {
                                ...
                                if (len == 1) {
                                    ...
                                    if (short_price < opts[i + 1].price) {
                                        opts[i + 1].price = short_price;
                                        opts[i + 1].pos_prev = i;
                                        opts[i + 1].back_prev = 0;
                                ...
                                if (rep_index == 1) rep_price += GetPrice(probs.is_rep_g1[state], 0);
                                    ...
                                    rep_price += GetPrice(probs.is_rep_g1[state], 1) + GetPrice(probs.is_rep_g2[state], rep_index - 2);
                                }
                            }
                            ...
                            for (uint32_t l = MIN_MATCH; l <= len; l++) {
                                ...
                                if (current_rep_price < opts[i + l].price) {
                                    opts[i + l].price = current_rep_price;
                                    opts[i + l].pos_prev = i;
                                    opts[i + l].back_prev = rep_index;
                ...
                // 3. Normal LZ Match Option with literal lookahead
                uint32_t normal_match_price = match_prob_price + GetPrice(probs.is_rep[state], 0);
                for (uint32_t l = MIN_MATCH; l <= max_limit; l++) {
                    uint32_t best_dist_for_l = 0xFFFFFFFF;
                    uint32_t best_lookahead_price = RC_INFINITY_PRICE;
                    uint32_t best_m_price = RC_INFINITY_PRICE;
                    uint32_t prev_dist = 0xFFFFFFFF;

                    // Evaluate distances sorted by optimal bits matched at boundary (7 down to 0)
                    for (int b = 7; b >= 0; b--) {
                        ...
                        uint32_t m_price = normal_match_price + GetLenPrice(probs.match_len, l, posState);
                        m_price += TreePrice(&probs.pos_slot[lenState * 64], 6, slot);
                        ...
                        if (slot >= 4) {
                            ...
                            if (slot < 14) m_price += ReverseTreePrice(&probs.pos_spec_tree[(slot - 4) * 64], footerBits, directVal);
                                ...
                                m_price += (footerBits - 4) * 128;
                                m_price += ReverseTreePrice(probs.prob_align, 4, directVal & 15);
                            }
                        }

                        // Combine pure match cost + projected literal mismatch cost exactly
                        ...
                        if (l < max_limit) {
                            ...
                            uint8_t curByte2 = data[global_pos + l];
                            uint8_t matchByte2 = data[global_pos + l - dist];
                            uint8_t prevByte2 = data[global_pos + l - 1];
                        ...
                        if (lookahead_price < best_lookahead_price) {
                            best_lookahead_price = lookahead_price;
                            best_m_price = m_price;
                            best_dist_for_l = dist;
                        }
                    }

                    if (best_dist_for_l != 0xFFFFFFFF && best_m_price < opts[i + l].price) {
                        opts[i + l].price = best_m_price;
                        opts[i + l].pos_prev = i;
                        opts[i + l].back_prev = best_dist_for_l + REPS;
                        opts[i + l].state = state < 7 ? 7 : 10;
                        opts[i + l].backs[0] = best_dist_for_l;
            ...
            if (token.back_prev == 0xFFFFFFFF) { 
                ...
                if (current_state >= 7) { 
                    ...
                    for (int i = 7; i >= 0; i--) {
                        uint32_t matchBit = (matchByte >> i) & 1;
                        uint32_t bit = (symbol >> i) & 1;
                        rc.EncodeBit(lit_probs[(0x100 + (matchBit << 8)) + m], bit);
                        m = (m << 1) | bit;
                        if (matchBit != bit) {
                            i--;
                            for (; i >= 0; i--) {
                                uint32_t b = (symbol >> i) & 1;
                                rc.EncodeBit(lit_probs[m], b);
                                m = (m << 1) | b;
                            }
                            break;
                        }
                ...
                rc.EncodeBit(probs.is_match[current_state * numPosStates + posState], 1);
                rc.EncodeBit(probs.is_rep[current_state], 1);
                if (token.back_prev == 0) {
                    ...
                    if (token.len == 1) { 
                        ...
                        current_state = current_state < 7 ? 9 : 11;
                        global_pos++;
                        continue;
                    }
                ...
                current_reps[0] = dist;
                current_state = current_state < 7 ? 8 : 11;
                global_pos += token.len;
            } else {
                rc.EncodeBit(probs.is_match[current_state * numPosStates + posState], 1);
                rc.EncodeBit(probs.is_rep[current_state], 0);
                rc.EncodeLen(probs.match_len, token.len, posState);

                uint32_t dist = token.back_prev - REPS;
                uint32_t distSymbol = dist - 1;
                uint32_t lenState = std::min(token.len - 2, (uint32_t)3);
                uint32_t slot = GetPosSlot(distSymbol);
                ...
                current_reps[0] = dist;
                current_state = current_state < 7 ? 7 : 10;
                global_pos += token.len;
            }
        }
...
void print_help(const char* prog_name) {
    std::cout << "Usage: " << prog_name << " [OPTION]... [FILE]...\n"
              << "Optimal LZMA compression evaluating fractional bit-length DP paths.\n\n"
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-c" || arg == "--stdout") to_stdout = true;
            ...
            if (!parse_lzma_args(arg, params)) { std::cerr << argv[0] << ": invalid lzma parameter string\n"; return 1; }
        }
        else if (arg[0] == '-' && arg != "-") { std::cerr << argv[0] << ": unrecognized option '" << arg << "'\n"; return 1; }
